...
export function buildPasswordResetEmail(recipient, resetUrl) {
  const html = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#0f0f14;font-family:Georgia,serif;">
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#18181f;border-radius:16px;overflow:hidden;border:1px solid #2a2a35;">
        <tr>
          <td style="padding:32px 40px;background:linear-gradient(135deg,#1a1a24,#0f0f14);border-bottom:1px solid #2a2a35;">
            <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.3em;color:#d4af37;text-transform:uppercase;">Sneaker Vault</p>
            <h1 style="margin:0;font-size:28px;color:#fafafa;font-weight:normal;">Reset Your Password</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:32px 40px;">
            ...
            <p style="margin:0 0 24px;color:#a1a1aa;font-size:15px;line-height:1.6;">
              We received a request to reset the password for your Sneaker Vault account. Click the button below to set a new password.
            </p>
            <div style="margin:32px 0;text-align:center;">
              <a href="${resetUrl}" style="display:inline-block;background:#d4af37;color:#0a0a0f;padding:14px 28px;border-radius:999px;text-decoration:none;font-size:14px;font-weight:600;">Reset Password</a>
            </div>
            <p style="margin:0 0 24px;color:#71717a;font-size:13px;line-height:1.5;">
              This link is valid for <strong>1 hour</strong>. If you did not request this, you can safely ignore this email — your password will remain unchanged.
            </p>
          </td>
        </tr>
        <tr>
  ...
  const text = `Hi ${recipient.name || 'there'},\n\nWe received a request to reset your password. Please use the following link to set a new one:\n${resetUrl}\n\nThis link is valid for 1 hour.`;
...
export async function sendPasswordResetEmail(recipient, resetUrl) {
  const { subject, html, text } = buildPasswordResetEmail(recipient, resetUrl);
  return sendMail({ to: recipient.email, subject, html, text });
}
...
export function buildBackInStockEmail(email, product) {
...
<body style="margin:0;padding:0;background:#0f0f14;font-family:Georgia,serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f0f14;padding:40px 20px;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#18181f;border-radius:16px;overflow:hidden;border:1px solid #2a2a35;">
        <tr>
          <td style="padding:32px 40px;background:linear-gradient(135deg,#1a1a24,#0f0f14);border-bottom:1px solid #2a2a35;">
            <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.3em;color:#d4af37;text-transform:uppercase;">Sneaker Vault Restock</p>
            <h1 style="margin:0;font-size:28px;color:#fafafa;font-weight:normal;">Back In Stock!</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:32px 40px;text-align:center;">
            <p style="margin:0 0 24px;color:#a1a1aa;font-size:16px;line-height:1.6;text-align:left;">
              Great news! A pair you've been eyeing is back in the vault and ready for you. Don't wait — stock is highly limited.
            </p>
            ...
            <div style="background:#0f0f14;border-radius:12px;padding:24px;border:1px solid #2a2a35;margin-bottom:32px;display:inline-block;width:100%;box-sizing:border-box;">
              ...
              <p style="margin:0 0 16px;color:#d4af37;font-size:18px;font-weight:bold;">$${product.price}</p>
              <a href="${productUrl}" style="display:inline-block;background:#d4af37;color:#0a0a0f;padding:12px 24px;border-radius:999px;text-decoration:none;font-size:14px;font-weight:600;">Grab Yours Now</a>
            </div>
          </td>
        </tr>
        <tr>
          <td style="padding:20px 40px 32px;border-top:1px solid #2a2a35;">
            <p style="margin:0;font-size:11px;color:#52525b;line-height:1.5;text-align:center;">
              You received this because you subscribed to a stock alert for this product.
            </p>
          </td>
        </tr>
      </table>
  ...
  return {
    subject: `🔥 Restock Alert: ${product.name} is Back in Stock!`,
    html,
    text
  };
}
...
export async function sendBackInStockEmail(email, product) {
  const { subject, html, text } = buildBackInStockEmail(email, product);
  return sendMail({ to: email, subject, html, text });
}
